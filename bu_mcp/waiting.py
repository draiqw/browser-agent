...
async def wait_for_page_ready(session: Any, *, timeout: float = 8.0) -> dict:
	...
	for name, runner, cap in ladder:
		...
		if budget <= 0.05:
			stages.append({'name': name, 'ok': False, 'elapsed': 0.0, 'detail': 'skipped: overall timeout budget exhausted'})
...
async def _selfcheck() -> int:
	...
	try:
		...
		# --- 3. тяжёлая SPA --------------------------------------------------
		nav = session.event_bus.dispatch(NavigateToUrlEvent(url='https://github.com/browser-use/browser-use', new_tab=False))
		await nav
		res = await wait_for_page_ready(session)
		...
		res = await wait_for_page_ready(session)
		_print_result(
			'4/4 бесконечная фоновая активность (example.com + вечный setInterval/fetch)',
			await _current_url(session, my_tab),
			res,
		)
		if res['ready']:
