...
Журналируется как элементное действие; `to_macro` выносит имя файла в
переменную `attachment`, `macro.run` резолвит её обратно внутри папки.

Обучение -> скрипт: `macro_record(action=start)` ставит закладку, `stop` собирает
макрос из отрезка журнала между закладками и сохраняет (`replace_from=N` —
починка: шаги `1..N-1` берутся из уже сохранённого макроса, хвост заменяется).
`macro_run(from_step=N)` продолжает с шага N; `new_tab=true` — в своей фоновой
вкладке. Если первый исполняемый шаг — не навигация, а браузер стоит на другой
странице, повтор сам открывает `url_before` этого шага (`auto_start`); этот адрес
входит в доменный гейт `_macro_urls`.
