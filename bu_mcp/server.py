...
* ``scroll``  — позиция прокрутки снимается ДО и ПОСЛЕ. У browser-use текстового
  ...
  статус ``at-end`` (см. ``bu_mcp.actions.scroll``).
...
подробности и вторая ступень — у ``bu_mcp.delta``.
...
* ``bu_mcp.server_shared``   — константы, ошибки, JS-сниппеты, схемы инструментов;
* ``bu_mcp.domain_gate``     — allowlist доменов (``DomainGateMixin``);
* ``bu_mcp.cdp_session``     — жизненный цикл CDP-сессии (``CdpSessionMixin``);
* ``bu_mcp.registry_bridge`` — мост к ``Tools()`` (``RegistryBridgeMixin``);
* ``bu_mcp.noop``            — контракт ActionResult (``NoopMixin``);
* ``bu_mcp.delta``           — расписка о последствиях (``DeltaMixin``);
* ``bu_mcp.actions.*``       — обработчики самих MCP-инструментов, по одному
  модулю на группу (navigate, interact, capture, scroll, switch, files, macros);
* ``bu_mcp.journal`` / ``bu_mcp.macro`` — журнал действий и повтор макросов.
...
from bu_mcp.actions.capture import CaptureActionsMixin
from bu_mcp.actions.files import FilesActionsMixin
from bu_mcp.actions.interact import InteractActionsMixin
from bu_mcp.actions.macros import MacroToolsMixin
from bu_mcp.actions.navigate import NavigateActionsMixin
from bu_mcp.actions.scroll import ScrollActionsMixin
from bu_mcp.actions.switch import SwitchActionsMixin
from bu_mcp.cdp_session import CdpSessionMixin
from bu_mcp.delta import DeltaMixin
from bu_mcp.domain_gate import DomainGateMixin
from bu_mcp.noop import NoopMixin
from bu_mcp.registry_bridge import RegistryBridgeMixin
from bu_mcp.server_shared import BRIDGE_EXCLUDE, JOURNALED_TOOLS, SECURITY_BOUNDARY, ToolError, bu_mcp_module
from bu_mcp.server_shared import JOURNAL_FIELDS as JOURNAL_FIELDS  # re-exported: bu_mcp.smoke reads it off this module
