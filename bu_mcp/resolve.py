...
async def _is_reachable(session: BrowserSession, node: EnhancedDOMTreeNode) -> bool:
	...
	* ``resolveNode`` отдаёт объект, но ``isConnected === false`` — узел ещё жив
	...
	try:
		...
		try:
			await cdp.cdp_client.send.Runtime.releaseObject(params={'objectId': object_id}, session_id=session_id)
		except Exception:
			pass
...
# Взаимозаменяемые контролы ввода текста. Один и тот же логический элемент сайт
# может отдавать то как ``<textarea>``, то как ``<div contenteditable>`` (или
# ``<input>``) в зависимости от состояния или A/B-ветки: ровно так ведёт себя
# композер ChatGPT — по умолчанию contenteditable-div, после фокуса textarea.
# Строгая проверка тега тогда отвергает верный элемент с тем же aria-label.
_EDITABLE_TAGS = frozenset({'textarea', 'input'})
_EDITABLE_ROLES = frozenset({'textbox', 'searchbox', 'combobox'})


def _node_editable(node: EnhancedDOMTreeNode) -> bool:
	"""Узел — это поле ввода текста, каким бы тегом он ни был отрисован."""
	if (node.node_name or '').lower() in _EDITABLE_TAGS:
		return True
	attrs = node.attributes or {}
	if 'contenteditable' in attrs and (attrs.get('contenteditable') or '').strip().lower() not in ('false',):
		return True
	ax = getattr(node, 'ax_node', None)
	role = ((ax.role if ax else None) or attrs.get('role') or '').lower()
	return role in _EDITABLE_ROLES


def _ident_editable(ident: dict[str, Any]) -> bool:
	"""То же по запомненному хендлу. Атрибуты хендла — только ``_IDENTIFYING_ATTRS``,
	поэтому опираемся на тег и роль (``contenteditable`` в них не попадает, но
	у contenteditable-элемента роль ``textbox`` обычно вычислена в AX)."""
	if (ident.get('tag') or '').lower() in _EDITABLE_TAGS:
		return True
	role = ((ident.get('role') or '') or (ident.get('attributes') or {}).get('role') or '').lower()
	return role in _EDITABLE_ROLES
...
def _tag_compatible(node: EnhancedDOMTreeNode, ident: dict[str, Any]) -> bool:
	...
	Применяется ТОЛЬКО на ступенях с сильным человекочитаемым якорем (accessible
	name, ``data-*``/``aria-label``): там совпадение и без тега надёжно. Для
	``id``/``name`` остаётся строгий ``_tag_ok`` — их подмена тега сомнительна."""
	if _tag_ok(node, ident):
		return True
	return _ident_editable(ident) and _node_editable(node)


def same_control_class(hint: dict[str, Any], node: EnhancedDOMTreeNode) -> bool:
	"""Публично для ``macro._identity_mismatch``: расходятся ли теги допустимо.

	``True`` — теги совпали ИЛИ оба элемента из класса взаимозаменяемых полей
	ввода. Только тогда клик/ввод по переидентифицированному узлу безопасен,
	несмотря на другой тег."""
	tag = (hint.get('tag') or '').lower()
	got = (getattr(node, 'node_name', '') or '').lower()
	if not tag or not got or tag == got:
		return True
	return _ident_editable(hint) and _node_editable(node)
...
def _candidates_for_level(
	...
	if level == 'backend_node_id':
		...
		found = [
			(i, n)
			for i, n in items
			if n.backend_node_id == want and (not ident.get('session_id') or str(n.session_id) == ident['session_id'])
		]
	...
	if level == 'accessible_name':
		...
		for i, n in items:
			...
			if ax is None or ax.name != want:
				continue
			# Сильный якорь (совпал accessible name) — тег может отличаться внутри
			# класса полей ввода: textarea <-> contenteditable-div.
	...
	if level == 'attribute':
		...
		for key in _IDENTIFYING_ATTRS:
			...
			if key in _VOLATILE_ATTRS and _looks_generated(value):
				...
				continue
			# Человекочитаемые якоря (data-*, aria-label) достаточно сильны, чтобы
			# допустить подмену тега внутри класса полей ввода; volatile id/name —
			# нет, там тег обязан совпасть.
			tag_check = _tag_ok if key in _VOLATILE_ATTRS else _tag_compatible
			found = [(i, n) for i, n in items if tag_check(n, ident) and (n.attributes or {}).get(key) == value]
			if found:
...
if __name__ == '__main__':
	...
	async def main() -> int:
		...
		try:
			...
			print(
				f'  до пересоздания: index={target_index} backendNodeId={before["backend_node_id"]} '
				f'xpath={before["xpath"]} reachable={before["reachable"]}'
			)

			await evaluate(
				session, "(() => { const el = document.querySelector('a'); el.outerHTML = el.outerHTML; return true; })()"
			)
			...
			legacy_alive = await _is_reachable(session, legacy_node) if legacy_node is not None else None
			print(
				f'  browser-use get_dom_element_by_index({target_index}) -> '
			...
			snapshot_handles(session)
			selector_map = session._cached_selector_map or {}
			gone_index = next((i for i, n in selector_map.items() if (n.node_name or '').lower() == 'a'), None)
			...
			snapshot_handles(session)
			selector_map = session._cached_selector_map or {}
			url_index = next((i for i, n in selector_map.items() if (n.node_name or '').lower() == 'a'), None)
